...
%     PopObj(any(PopObj>1,2),:) = [];
    ...
    nadir = max(PF, [], 1);
    % 判断种群是否有至少有一个解支配nadir，如果是，则计算-HV，不是则计算Dist
    isDominateNadir = false;

    if size(PopObj,2) ~= size(PF,2)
        Score = nan;
    elseif M < 4
        % 初始化一个逻辑索引数组，用于标记需要保留的行
        keepRows = false(size(PopObj, 1), 1);
        for i = 1:size(PopObj,1)
            k = any(PopObj(i,:)<nadir) - any(PopObj(i,:)>nadir);
            if k == 1  %种群中解i支配nadir
                isDominateNadir = true;
                keepRows(i) = true;
            end
        end
        if isDominateNadir
            PopObj = PopObj(keepRows, :);
            ...
            for k = 1 : M-1
                S_ = {};
                for i = 1 : size(S,1)
                    Stemp = Slice(cell2mat(S(i,2)),k,nadir);
            ...
            for i = 1 : size(S,1)
                ...
                Score = Score + cell2mat(S(i,1))*abs(p(M)-nadir(M));
            end
            Score = - Score;
        else
            trueDist = [];
            for i = 1:size(PopObj,1)
                k = any(PopObj(i,:)<nadir) - any(PopObj(i,:)>nadir);
                if k == -1  %nadir支配该解，计算欧式距离
                    trueDist = [trueDist,sqrt(sum((PopObj(i,:) - nadir).^2))];
                else
                    tmp = PopObj - nadir;
                    tmp = tmp(tmp>0);
                    trueDist = [trueDist,min(tmp)];
                end
            end
            Score = min(trueDist);
        end
